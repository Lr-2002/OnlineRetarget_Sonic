...
class A0FrozenAEConfigTests(unittest.TestCase):
    ...
    def test_a0_visual_acceptance_backend_has_cli_and_real_backend_markers(self) -> None:
        ...
        self.assertIn("visual_renderer.backend_manifest(active_backend=PRIMARY_VISUAL_BACKEND)", script_text)
        self.assertIn("accepted_vertical_v2", script_text)
        self.assertIn("vertical_somamesh_g1target_g1kinematics", script_text)
        self.assertIn("row1_soma_somamesh.mp4", script_text)
        self.assertIn("row2_g1_target_isaaclab.mp4", script_text)
        self.assertIn("row3_g1_kinematics_isaaclab.mp4", script_text)
        self.assertIn('"panel_order": ["Soma", "G1 Target Playback", "G1 Kinematics Playback"]', script_text)
        self.assertIn('"skeleton_fallback_used": False', script_text)
        self.assertIn('"mesh_skinning_metadata": source_report.get("mesh_skinning_metadata", {})', script_text)
        self.assertIn('"backend": "IsaacLab"', script_text)
        self.assertIn('"motion_sha256": target_motion_asset_report["sha256"]', script_text)
        self.assertIn('"motion_sha256": motion_asset_report["sha256"]', script_text)
        self.assertIn('"checkpoint": str(cfg.get("checkpoint_path", ""))', script_text)
        self.assertIn('layout="vertical"', script_text)
        self.assertIn("vstack", script_text)
        self.assertIn("02_g1_target_playback_isaaclab_input.npz", script_text)
        self.assertIn("03_g1_kinematics_playback_isaaclab_input.npz", script_text)
