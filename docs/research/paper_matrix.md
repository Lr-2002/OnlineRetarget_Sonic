...
| BeyondMimic | From Motion Tracking to Versatile Humanoid Control via Guided Diffusion | https://arxiv.org/abs/2508.08241 and https://github.com/HybridRobotics/whole_body_tracking | PDF text deep-read, matrixed |
| MaskedMimic | Unified Physics-Based Character Control Through Masked Motion Inpainting | https://arxiv.org/abs/2409.14393 | PDF text deep-read, matrixed |
| PHC | Perpetual Humanoid Control for Real-time Simulated Avatars | https://arxiv.org/abs/2305.06456 | PDF text deep-read, matrixed |
| ProtoMotions-G1 | G1 Whole-Body Tracker workflow | https://github.com/NVlabs/ProtoMotions and https://protomotions.github.io/ | README/docs deep-read, matrixed |
| TMR | Text-to-Motion Retrieval Using Contrastive 3D Human Motion Synthesis | https://arxiv.org/abs/2305.00976 | PDF text deep-read, matrixed for motion clustering |
...
| Tracking/latent/contact deep-read | BeyondMimic, MaskedMimic, PHC, ProtoMotions, TMR, and contact-metric formulas | `docs/research/papers/tracking_latent_contact_deep_read.md` | Added to close the main M1 design gaps |
...
| BeyondMimic | Reference phase, anchor pose error, IMU/base twist, joint positions relative to default, joint velocities, previous action, and future/reference body features | Normalized joint position setpoints / PD targets; later latent state-action diffusion outputs decoded actions | PPO tracking policy plus VAE and guided diffusion for versatile control | Retargeted humanoid/G1-like motion references; paper reports about 2.5 hours of diverse motions | Body position/orientation/linear-velocity/angular-velocity tracking rewards, soft joint-limit/action-rate/self-contact regularizers, VAE reconstruction/KL, diffusion denoising loss | Motion tracking success, real deployment, guided diffusion tasks, inpainting, joystick/velocity/obstacle tasks | Strong simulator-eval style and obs contract evidence. Do not start baseline with diffusion under the 1 ms target; borrow anchor/root-relative body metrics and action smoothness. |
| MaskedMimic | Teacher observes current humanoid state, next K target poses, terrain/object context; student observes masked future pose/text/object goals plus current/past state tokens | Gaussian policy actions / PD control targets | Transformer tracker plus CVAE-style partially constrained controller with encoder/prior/decoder | Large mocap clips with BABEL/HumanML3D text labels and object/terrain contexts | Teacher RL reward over global joint positions, rotations, root height, velocities, angular velocities, and energy; student action likelihood plus encoder-prior KL under structured masks | Full-body tracking, VR/sparse tracking, path following, text commands, object interaction | Useful later for robustness and partial-observation ablations. First baseline should not inherit this complexity. |
| PHC | Proprioception plus goal state normalized by current facing/root; optional body-shape features; goal can be rotation or keypoint differences | PD targets for SMPL humanoid joints | Progressive Multiplicative Control Policy with PPO, AMP, hard-negative mining, and fail-state recovery | AMASS-scale human motion and noisy video/language generated motion | Imitation reward over body translation, rotation, linear velocity, angular velocity, AMP style reward, and energy penalty | AMASS imitation success, fail-state recovery, noisy-input/avatar control | Keep morphology/body-shape conditioning in M3/M5. Avoid over-deleting dynamic clips only because feet/toes do not match human contact exactly. |
| ProtoMotions-G1 | Real-hardware-compatible sensor observations: joint pos/vel, torso IMU orientation, pelvis angular velocity, previous action, future motion reference frames | ONNX policy outputs PD position targets, with observation computation/action processing baked into export | General tracking policy framework across IsaacGym, IsaacLab, Newton, MuJoCo; supports retargeting and deployment | AMASS and BONES-SEED, including full BONES-SEED G1 CSV converted to MotionLib | Configurable tracking rewards; README example uses heading velocity direction/facing reward, soft limits, action smoothness in tracking setups | Sim2sim validation, MuJoCo/G1 deployment, real Unitree G1 zero-shot transfer | Export observation schema and normalization with model weights. Keep BONES-SEED SEED/G1 preparation as first-class artifacts. |
...
| TMR | Text and motion encoded into shared latent space; motion preprocessing includes canonicalization, local positions, velocities, and foot contacts | Retrieval similarity scores and optional reconstructed motion | TEMOS-like text/motion encoders plus non-autoregressive decoder and contrastive InfoNCE | HumanML3D and KIT-ML | Reconstruction/KL/cross-modal embedding losses plus weighted InfoNCE; filters near-duplicate text negatives | Text-to-motion and motion-to-text recall@K, median rank, gallery protocols | Not a retargeter, but useful for behavior-balanced review or semantic clustering so manual review does not over-sample near-duplicate motions. |
| Contact and Human Dynamics | Initial 2D/3D pose estimates plus predicted foot-ground contact timings | Contact-aware physically plausible full-body motion | Contact prediction network plus physics-based trajectory optimization | Monocular dynamic video sequences and contact-supervised/synthetic evidence | Contact/physics trajectory optimization; metrics rather than supervised retarget loss | Floating, penetration, skate, GRF, global/body/foot position errors | Use concrete contact formulas in M2Q/M4: contact-foot float above 3 cm, penetration below 3 cm, skate over 2 cm horizontal contact displacement. |
