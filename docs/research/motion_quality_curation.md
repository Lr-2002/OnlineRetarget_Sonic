...
| Support-base / CoM plausibility | source, G1 sim/FK | Pending | Pelvis/root projection relative to support polygon when contact is known; PHUMA uses pelvis/spine distance to base-of-support checks after contact-ground alignment | `quarantine`; review category-specific exceptions |
| Pelvis/root height plausibility | source, G1 FK | Pending | Min/max pelvis or root height after consistent ground-plane estimation; PHUMA uses pelvis-height bounds to catch excessive crouch/lying/floating | `quarantine` pending category review; `exclude` only for confirmed invalid global placement |
...
11. Add support-base/pelvis-height scans before promoting a policy beyond the current representative evidence. Acceleration/jerk metrics now exist but still need BONES-SEED-calibrated thresholds.
