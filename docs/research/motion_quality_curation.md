...
- Manual-review decision ingest is implemented via `merge-review-decisions`. Reviewer CSV/JSONL rows are keyed by `review_id` and must provide `decision` plus a legal `recommended_action` in `keep`, `downweight`, `quarantine`, or `exclude`. The command writes a new reviewed manifest and `review_decision_report.json`; it never overwrites the original review sheet. `audit-curation-policy` can record the decision report and still blocks promotion if any review items are incomplete or carry invalid actions.
