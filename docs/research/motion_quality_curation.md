...
| Contact-mask correction candidate | source, G1 FK | Pending | Identify clips where contact labels imply a simple vertical correction could repair minor floating without destroying dynamics, following KungfuBot-style contact-mask correction | `quarantine` until corrected and reviewed; never silently rewrite source data |
