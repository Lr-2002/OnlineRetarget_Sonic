...
| DynaRetarget | Missing contacts, penetration/skating-like kinematic imperfections, discontinuous contact/object changes, poor reference quality | Refines imperfect kinematic demonstrations with MuJoCo rollout-based sampling trajectory optimization; reports refinement success, smoothness, compute efficiency, and downstream policy benefit. | M7 should record simulator-refinement success/failure as a quality label. Failed refinement is evidence for quarantine even if kinematic metrics look acceptable. |
| SPIDER | Noisy kinematic demonstrations, missing force/contact data, contact ambiguity, and dynamically infeasible direct IK transfer | Uses physics-based sampling with annealed kernels and virtual contact guidance; filters unstable short/drifting contacts before applying guidance. | Use as evidence that contact-sequence correctness and refinement convergence belong in simulator-stage quality metadata. |
| ReActor | Source floating/penetration, ground penetration, self-penetration, foot sliding, foot floating, abrupt joint movements | Uses sparse semantic correspondences and RL-coupled retargeting; reports kinematic metrics for ground/self penetration, foot sliding, and foot floating, plus action/torque/acceleration regularization. | Borrow metric definitions and keep torque/action smoothness as simulator-stage quality signals. |
| KungfuBot | Video reconstruction errors, motions beyond physical limits, missing contact labels, minor floating, correction jitter | Multi-step motion processing: physics-based filtering, contact-mask extraction, contact-aware vertical correction, EMA smoothing, and IK retargeting with joint-limit respect. | Add a future correction lane for quarantined clips; dynamic motions need thresholds that preserve high-energy behavior. |
| RoboForge | Generated motions with physical infeasibility, contact-transition errors, skating, floating, penetration | Simulation-based physical plausibility optimization; quality control retains stable refined rollouts and discards failures. | M7 should support a generate/execute/filter/retrain loop, with refined rollouts labeled separately from kinematic G1 CSV targets. |
...
| Acceleration/jerk | source, G1 | Metric-only scanner implemented; thresholds pending | Second/third finite differences after unit normalization; PHUMA reports root jerk as a formal filtering metric and KDMR flags high acceleration as actuator-infeasible | Record metrics by default. `quarantine` only when explicit calibrated thresholds are configured; category-aware for jumps/fights |
...
| Support-base / CoM plausibility | source, G1 sim/FK | Metric-only scanners implemented for source FK and G1 MJCF FK; thresholds pending | Pelvis/root projection relative to contact foot support points in the horizontal plane; PHUMA uses pelvis/spine distance to base-of-support checks after contact-ground alignment | Record `support_frame_ratio`, `mean_root_support_distance`, and `max_root_support_distance` by default. Promote to `quarantine` only after BONES-SEED category/skeleton calibration and review of airborne/sit/get-up exceptions |
| Pelvis/root height plausibility | source, G1 FK | Metric-only scanners implemented for source FK and G1 CSV/MJCF paths; thresholds pending | Min/max/range/mean pelvis or root height after consistent ground-plane estimation; PHUMA uses pelvis-height bounds to catch excessive crouch/lying/floating | Record metrics by default. `quarantine` pending category review; `exclude` only for confirmed invalid global placement |
| Contact-mask correction candidate | source, G1 FK | Metric-only scanners implemented; correction itself pending | Identify clips where contact labels imply a bounded vertical correction could repair minor floating or penetration without destroying dynamics, following KungfuBot-style contact-mask correction | Record candidate/offset/reason in quality stats and manual-review manifests. `quarantine` until corrected and reviewed; never silently rewrite source data |
| Simulator refinement outcome | G1 sim, later source-to-G1 pair | Pending | Replay or track representative clips in Isaac/MuJoCo; record success/fall, tracking error, contact artifacts, contact-sequence correctness, compute budget, and whether refinement converged, following OmniTrack, NMR/CEPR, RoboForge, DynaRetarget, and SPIDER | `quarantine` failed refinement; `downweight` partial success; promote refined outputs only under separate provenance |
...
12. Paper thresholds are evidence, not defaults. PHUMA's reported root jerk, foot-contact, pelvis-height, and support-base thresholds justify which signals to add, but OnlineRetarget thresholds must be calibrated on BONES-SEED distributions before they become a policy.
...
The 2026-05-14 PDF/OpenAlex deep read is recorded in `docs/research/papers/motion_filtering_deep_read.md`. The main update from that pass is that data filtering should expand beyond existing source/G1/pair scanners to include jerk, support-base, pelvis/root-height, contact-mask correction candidates, and simulator-refined provenance.

The first follow-up implementation adds source and G1 acceleration/jerk metrics as measurement-only signals. The scanner emits `max_abs_channel_acceleration`, `max_root_acceleration`, `max_root_jerk`, `max_abs_joint_acceleration`, and G1 root acceleration/jerk fields; flags are only emitted when optional thresholds are passed on the CLI. This keeps paper-derived signals visible without silently promoting uncalibrated thresholds.

The next implementation adds support-base and pelvis/root-height metrics as measurement-only signals. Source FK quality now records `root_height_min`, `root_height_max`, `root_height_range`, `mean_root_height`, `support_frame_ratio`, `mean_root_support_distance`, and `max_root_support_distance` using `Hips` and contact foot points in the BVH horizontal plane. G1 quality records CSV root-height min/max/range and, when an MJCF model is supplied, FK support-frame and root-to-support distance metrics using contact foot bodies. `merge-quality` carries these fields into `worst_clips.csv` so review sheets can see support/height artifacts without making them default filters.

The current contact-mask correction update is still measurement-only: source FK and G1 FK scans emit `contact_correction_candidate`, `contact_correction_offset`, and `contact_correction_abs_offset` for bounded vertical float/penetration fixes; source FK also emits a human-readable `contact_correction_reason`. These fields are copied into `worst_clips.csv` and manual-review manifests so reviewers can decide whether a clip should be repaired, retained, or kept quarantined. No scanner rewrites source data or promotes corrected targets.

The latest source inspection also checked released PHUMA code, not only paper summaries. PHUMA's curation script uses 4-second chunks, 0.5-second overlap, low-pass filtering, robust ground estimation from foot vertices, foot-contact scoring, mean root jerk, pelvis-height bounds, and pelvis/spine distance to a foot/ankle support polygon. Its public defaults are useful anchors for signal design, but OnlineRetarget must still calibrate thresholds on BONES-SEED because our source skeletons, G1 target format, FPS, and action distribution differ.

The filtering answer is therefore operational: first measure source, G1, pair, and provenance quality independently; then generate category/actor/skeleton threshold proposals; then review worst clips and diversity loss; then optionally run simulator refinement on representative ambiguous clips; only then promote a named policy ID for formal training.
...
- Accepted threshold policies are now separate artifacts, not a CLI-only promise. `accept-threshold-policy` wraps reviewed proposal JSON files into a named policy JSON with `policy_id`, `accepted_by`, `rationale`, proposal summaries, git SHA, and whether the policy is only representative. `audit-curation-policy` can consume `--threshold-policy-json` and still blocks promotion if scan coverage, review decisions, diversity, or policy ID checks fail.
...
- Manual-review decision ingest is implemented via `merge-review-decisions`. Reviewer CSV/JSONL rows are keyed by `review_id` and must provide `decision` plus a legal `recommended_action` in `keep`, `downweight`, `quarantine`, or `exclude`. The CSV generated by `build-review-decision-template` is directly ingestible after a reviewer fills those decision fields. The merge command writes a new reviewed manifest and `review_decision_report.json`; it never overwrites the original review sheet. `audit-curation-policy` can record the decision report and still blocks promotion if any review items are incomplete or carry invalid actions.
