...
| Foot float | source, G1 FK | Pending body/FK extraction | Estimate ground plane and foot height during contact; start with heel/toe/ankle proxies | `quarantine`; `exclude` if clip is globally floating like invalid sit/lie |
| Foot slide/skate | source, G1 FK | Pending contact labels | Horizontal foot velocity while contact confidence is high | `downweight` mild, `quarantine` severe |
| Ground penetration | source, G1 FK | Pending FK/ground plane | Minimum foot/body height below ground plane | `quarantine` mild, `exclude` severe |
