...
| Joint-limit margin | G1 | Pending G1 asset/joint-limit table | Load G1 joint limits from Isaac/MuJoCo asset; count violations and near-limit frames | `quarantine` for mild margin stress, `exclude` for severe violations |
| Foot float | source, G1 FK | Source FK smoke scanner implemented; G1 FK pending | Estimate ground plane and foot height during contact; source scanner supports fixed ground or foot-percentile ground | `quarantine`; `exclude` if clip is globally floating like invalid sit/lie |
| Foot slide/skate | source, G1 FK | Source FK smoke scanner implemented; G1 FK pending | Horizontal foot velocity while contact confidence is high; currently `scan-source-fk-quality` uses foot height contact heuristics | `downweight` mild, `quarantine` severe |
| Ground penetration | source, G1 FK | Source FK smoke scanner implemented; G1 FK pending | Minimum foot/body height below ground plane; fixed `ground_height=0.0` keeps global float/penetration visible | `quarantine` mild, `exclude` severe |
