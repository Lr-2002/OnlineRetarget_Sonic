...
Latest unit-test evidence: 52 tests passed with `PYTHONPATH=src:. python3 -m unittest discover -s tests`.
...
| M2 Data inventory, curation, splits | Partial | `src/online_retarget/data/curation.py`, `src/online_retarget/data/g1_quality.py`, `src/online_retarget/data/bvh_quality.py`, `src/online_retarget/data/source_fk_quality.py`, `src/online_retarget/data/thresholds.py`, `src/online_retarget/data/quality_merge.py`, `src/online_retarget/data/row_sampling.py`, `src/online_retarget/data/review_manifest.py`, CLI `split-index`, `scan-g1-quality`, `scan-source-quality`, `scan-source-fk-quality`, grouped `propose-thresholds`, stratified `--sample-by`, `merge-quality`, `build-review-manifest`, related tests, real index and quality artifacts under `runs/` | M2Q quality gate is now explicit; source BVH, source FK/contact, and G1 FK/contact smoke stats now merge into the curated index, but self-intersection, larger/full quality scans, completed manual review decisions, and final calibrated thresholds still pending |
| M2Q Motion quality curation gate | Partial with enforceable train gate | `docs/research/motion_quality_curation.md`, existing source/G1 smoke scanners, source FK/contact smoke scanner, G1 MJCF FK/contact smoke scanner, grouped threshold-proposal support, stratified scan selection support, manual-review manifest support, merged curated index smoke artifact, `runs/curated/smoke_source_g1_limit100/worst_clips.csv`, curated report breakdown by split/package/category plus `diversity_loss`, and `scripts/train.py` formal-run quality gate. Current curated report records `merged_source_rows=100`, `merged_source_fk_rows=100`, and `merged_g1_rows=100`; smoke diversity-loss keeps 522/522 actor/source-skeleton groups represented. Current grouped-threshold evidence includes `runs/quality/actor_split_t80_v10_x10_s17_metadata_balanced_v0_source_fk_limit100/source_fk_threshold_proposals_grouped_p95.json` and `runs/quality/actor_split_t80_v10_x10_s17_metadata_balanced_v0_limit6/g1_threshold_proposals_grouped_p95.json`. Current stratified scan evidence includes `runs/quality/actor_split_t80_v10_x10_s17_metadata_balanced_v0_source_fk_limit24_by-category-split/source_fk_quality_report.json`. Current manual-review evidence includes `runs/curated/smoke_source_g1_limit100/manual_review/review_manifest.jsonl` and `.md`, covering parser/mirror/jump/slide/penetration/float/joint-limit families. Current plan explicitly follows NMR/CEPR, PHUMA, GMR, OmniTrack, OmniRetarget, KDMR, and contact-metric papers. | Need full or representative scans beyond smoke scale, category-aware thresholds from full/representative stats, completed manual review decisions, self-collision proxy, simulator-verified quality labels, and paper notes deep enough to justify each formal filter |
...
Latest G1 MJCF FK/contact smoke result: first 100 clips scanned with `/home/user/repos/GMR/assets/unitree_g1/g1_mocap_29dof.xml`, `frame_stride=2`, `max_frames=256`; keep/downweight/quarantine = 19/36/45, flags `g1_foot_slide=70`, `g1_ground_penetration=41`, `g1_joint_limit_violation=18`, `g1_unstable_start_end=8`, `g1_foot_float=1`. The refreshed three-way curated smoke index has keep/downweight/quarantine/exclude = 71,088/71,048/83/1 and includes `source_fk:source_foot_slide=20`, `source_fk:source_low_foot_contact=38`, source FK contact metrics in `worst_clips.csv`, and `diversity_loss` showing 0 lost actor/source-skeleton groups in this smoke policy.
