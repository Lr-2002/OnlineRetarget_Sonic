...
| M2Q Motion quality curation gate | Partial with enforceable train gate | `docs/research/motion_quality_curation.md`, existing source/G1 smoke scanners, source FK/contact smoke scanner, G1 MJCF FK/contact smoke scanner, threshold-proposal scaffold, merged curated index smoke artifact, `runs/curated/smoke_source_g1_limit100/worst_clips.csv`, curated report breakdown by split/package/category, and `scripts/train.py` formal-run quality gate. Current curated report records `merged_source_rows=100`, `merged_source_fk_rows=100`, and `merged_g1_rows=100`. Current plan explicitly follows NMR/CEPR, PHUMA, GMR, OmniTrack, OmniRetarget, KDMR, and contact-metric papers. | Need full or representative scans beyond first 100, per-category calibrated thresholds, actor/skeleton diversity-loss report, self-collision/manual review loop for worst clips, simulator-verified quality labels, and paper notes deep enough to justify each formal filter |
