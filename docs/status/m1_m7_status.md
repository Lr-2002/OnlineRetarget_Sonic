...
PYTHONPATH=src:. python3 scripts/train.py --config configs/baseline_mlp.yaml --samples-jsonl runs/supervised/train_merged-quality-action_h8_limit8/samples.jsonl --dry-run --limit 1
```

Latest unit-test evidence: 28 tests passed with `PYTHONPATH=src:. python3 -m unittest discover -s tests`.
...
| M2 Data inventory, curation, splits | Partial | `src/online_retarget/data/curation.py`, `src/online_retarget/data/g1_quality.py`, `src/online_retarget/data/bvh_quality.py`, `src/online_retarget/data/thresholds.py`, `src/online_retarget/data/quality_merge.py`, CLI `split-index`, `scan-g1-quality`, `scan-source-quality`, `propose-thresholds`, `merge-quality`, related tests, real index and quality artifacts under `runs/` | M2Q quality gate is now explicit; foot slide/float, penetration, self-intersection, contact, larger quality scans, and final calibrated thresholds still pending |
| M2Q Motion quality curation gate | Partial with enforceable train gate | `docs/research/motion_quality_curation.md`, existing source/G1 smoke scanners, threshold-proposal scaffold, merged curated index smoke artifact, `runs/curated/smoke_source_g1_limit100/worst_clips.csv`, curated report breakdown by split/package/category, and `scripts/train.py` formal-run quality gate | Need FK/contact metrics, full or representative scans beyond first 100, per-category calibrated thresholds, actor/skeleton diversity-loss report, and manual review loop for worst clips |
| M3 Dataset schema and observation contract | Implemented scaffold | `src/online_retarget/data/schema.py`, `tests/test_schema.py`, architecture docs list observation dim 1547 and output dim 29 | Actual frame-window extraction from SOMA/G1 files pending |
...
| M5 Direct supervised baseline | Partial | `scripts/train.py --dry-run` reads config, split/curated index, M2Q quality gate context, schema, git state, DDP rank/world size, and sample refs; `scripts/train.py --samples-jsonl ...` has a PyTorch optimizer loop for supervised JSONL samples; formal non-dry-run training now refuses to start without quality policy ID, quality report, curated index, and `merged_quality_action`; `src/online_retarget/data/supervised_builder.py` builds tiny raw-BVH-channel debug samples from either raw split index or merged quality index | Final 30-body tensor dataset, WandB run, checkpointing policy for real runs, and actual torch-environment training execution pending |
