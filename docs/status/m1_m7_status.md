...
Latest unit-test evidence: 54 tests passed with `PYTHONPATH=src:. python3 -m unittest discover -s tests`.
...
| M2 Data inventory, curation, splits | Partial | `src/online_retarget/data/curation.py`, `src/online_retarget/data/g1_quality.py`, `src/online_retarget/data/bvh_quality.py`, `src/online_retarget/data/source_fk_quality.py`, `src/online_retarget/data/thresholds.py`, `src/online_retarget/data/quality_merge.py`, `src/online_retarget/data/row_sampling.py`, `src/online_retarget/data/review_manifest.py`, CLI `split-index`, `scan-g1-quality`, `scan-source-quality`, `scan-source-fk-quality`, grouped `propose-thresholds`, stratified `--sample-by`, `merge-quality`, `build-review-manifest`, related tests, real index and quality artifacts under `runs/` | M2Q quality gate is now explicit; source BVH, source FK/contact, and G1 FK/contact/self-collision-proxy smoke stats now merge into the curated index, but larger/full quality scans, completed manual review decisions, simulator collision labels, and final calibrated thresholds still pending |
| M2Q Motion quality curation gate | Partial with enforceable train gate | `docs/research/motion_quality_curation.md`, existing source/G1 smoke scanners, source FK/contact smoke scanner, G1 MJCF FK/contact/self-collision-proxy smoke scanner, grouped threshold-proposal support, stratified scan selection support, manual-review manifest support, merged curated index smoke artifact, `runs/curated/smoke_source_g1_limit100/worst_clips.csv`, curated report breakdown by split/package/category plus `diversity_loss`, and `scripts/train.py` formal-run quality gate. Current curated report records `merged_source_rows=100`, `merged_source_fk_rows=100`, and `merged_g1_rows=100`; smoke diversity-loss keeps 522/522 actor/source-skeleton groups represented. Current G1 smoke reports `g1_self_collision_proxy=1` and manual review now includes a self-collision family. Current grouped-threshold evidence includes `runs/quality/actor_split_t80_v10_x10_s17_metadata_balanced_v0_source_fk_limit100/source_fk_threshold_proposals_grouped_p95.json` and `runs/quality/actor_split_t80_v10_x10_s17_metadata_balanced_v0_limit6/g1_threshold_proposals_grouped_p95.json`. Current stratified scan evidence includes `runs/quality/actor_split_t80_v10_x10_s17_metadata_balanced_v0_source_fk_limit24_by-category-split/source_fk_quality_report.json`. Current manual-review evidence includes `runs/curated/smoke_source_g1_limit100/manual_review/review_manifest.jsonl` and `.md`, covering parser/mirror/jump/slide/penetration/float/joint-limit/self-collision families. Current plan explicitly follows NMR/CEPR, PHUMA, GMR, OmniTrack, OmniRetarget, KDMR, and contact-metric papers. | Need full or representative scans beyond smoke scale, category-aware thresholds from full/representative stats, completed manual review decisions, simulator-verified collision/quality labels, and paper notes deep enough to justify each formal filter |
