...
`--preserve-world-root` remains available for explicit world-frame playback,
but the accepted default is root-zeroed relative XY so the target trajectory is
readable without relying on the old 28 m preserved-world render.
