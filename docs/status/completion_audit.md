...
Verdict: not complete. The repo now has runnable scaffolds, real BONES-SEED smoke and representative legacy-archive M2Q artifacts, a full BONES-SONIC NPZ index, live logs, review-demo traceability surfaces, a stronger M1 deep-read set, and quality-gated training entry points, but formal M1-M7 completion is blocked by SONIC-native quality curation, final 30-body dataset extraction, real training, real 4090 latency, and Isaac Lab simulator evaluation.

2026-05-15 correction: current SONIC work must use `/home/user/data/motion_data/bones_sonic`. Existing `g1.tar`, `soma_proportional.tar`, and BVH/G1 CSV review-video artifacts are legacy evidence only and must not be used as SONIC quality/video evidence. See `docs/status/sonic_data_source.md` and `docs/status/sonic_skeleton_semantics.md`.

2026-05-18 pause decision: SONIC visual-QA / full-curation is intentionally skipped as the current blocking milestone. This does not certify the dataset or promote a quality policy. It records that the source correction, IsaacLab G1 order correction, bounded SONIC smoke scan, and corrected full-length capsule videos are enough to move to train/test/eval split and small-baseline work. Durable summary: `docs/status/data_experience_summary_2026-05-18.md`.
...
| Implement SONIC data inventory | `src/online_retarget/data/bones_sonic.py`, `src/online_retarget/data/sonic_quality.py`, `src/online_retarget/data/sonic_review_clips.py`, CLI `build-sonic-index`, CLI `scan-sonic-quality`, CLI `export-sonic-review-clips`, `tests/test_bones_sonic.py`, `tests/test_sonic_quality.py`, `runs/indices/bones_sonic_index_full_v0/sonic_index_report.json`, `runs/quality/bones_sonic_index_full_v0_sonic_limit256_by-category-date/sonic_quality_report.json`, `runs/quality/bones_sonic_index_full_v0_sonic_limit512_by-category-date/sonic_quality_report.json`, `runs/review_clips/sonic_quality_3d_capsules_v3_isaaclab_order/README.md`, `docs/status/sonic_data_source.md`, `docs/status/sonic_skeleton_semantics.md`, `docs/status/data_experience_summary_2026-05-18.md` | Paused/deferred after source correction, bounded quality smoke, IsaacLab-order skeleton correction, and full-length SONIC capsule review. Full index found 142,220 NPZ files, 142,220 metadata matches, 522 actors, 71,088 mirrors, all 50 Hz, all schema `ok`. The corrected 256-row SONIC quality smoke records 186 keep, 47 downweight, 23 quarantine; the 512-row smoke records 384 keep, 78 downweight, 50 quarantine. Six corrected full-length capsule videos render OK. Full SONIC scan, threshold promotion, manual review, simulator labels, and formal curation policy remain pending and are no longer the current blocker. |
