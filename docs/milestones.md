...
Current status: metadata-level actor split and curation index are implemented. First source BVH and G1 target quality scanners exist for channel/joint/root discontinuity and nonfinite stats. Contact/penetration/self-intersection signals and calibrated thresholds are tracked by M2Q, so M2 is not fully closed.
