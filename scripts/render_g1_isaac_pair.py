...
def main() -> None:
    ...
    motion = _load_motion(args_cli.npz, args_cli.max_frames)
    ...
    joint_order = _joint_order(robot.joint_names)
    root_offset = motion["root_pos"][0].copy()
    frames_written = 0
    changed_frames = 0
    previous_frame = None
    frame_sums: list[int] = []

    with imageio.get_writer(args_cli.output, fps=motion["fps"], codec="libx264", quality=8) as writer:
        for frame_index in range(motion["frame_count"]):
            root_pos = motion["root_pos"][frame_index] - root_offset
            root_pos[2] = motion["root_pos"][frame_index, 2]
            ...
            target = torch.as_tensor(root_pos[None, :], dtype=torch.float32, device=sim.device)
            eye = target + torch.tensor([args_cli.camera_offset], dtype=torch.float32, device=sim.device)
    ...
    report = {
        ...
        "sonic_joint_names": list(SONIC_JOINT_NAMES),
...
def _load_motion(npz_path: Path, max_frames: int) -> dict[str, np.ndarray | float | int]:
    ...
    if max_frames > 0:
        ...
        joint_vel = joint_vel[:max_frames]
...
def _joint_order(robot_joint_names: list[str]) -> np.ndarray:
    missing = [name for name in robot_joint_names if name not in SONIC_JOINT_NAMES]
    if missing:
        raise RuntimeError(f"Robot has joints not present in SONIC order: {missing}")
    return np.asarray([SONIC_JOINT_NAMES.index(name) for name in robot_joint_names], dtype=np.int64)
